function stats = runEnsembleAnalysis(neurons, kernelBinWidth, binSize)
%
...
%   stats
    ...
    nCorrectTrials = NaN(length(neurons), 1);
    for iNeuron = 1 : length(neurons)
        nCorrectTrials(iNeuron) = length(neurons(iNeuron).trialSpecificEvents.correctLongTrialCuesOn); 
    end
    ...
    earlyTimes = time >= 0 & time <= 6; 
    midTimes = time >= 6 & time <= 12;  
    lateTimes = time >= 12 & time <= 18; 
    ...
    parfor iTrial = 1 : nTrials
        ...
        rsquared_0_6 = corrcoef(predictionLabels1(earlyTimes), time(earlyTimes));
        r2_0_6(iTrial) = rsquared_0_6(1,2) ^ 2;  
        
        rsquared_6_12 = corrcoef(predictionLabels1(midTimes), time(midTimes));
        r2_6_12(iTrial) = rsquared_6_12(1,2) ^ 2;  
        
        rsquared_12_18 = corrcoef(predictionLabels1(lateTimes), time(lateTimes));
        r2_12_18(iTrial) = rsquared_12_18(1,2) ^ 2;  
