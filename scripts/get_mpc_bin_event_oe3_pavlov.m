function events = get_mpc_bin_event_oe3_pavlov(event_datapath, mpcParsed)
...
% C2  -> 12 -> 4
% C1  -> 11 -> 5 
eventData = readOEphys(event_datapath, 'events');
...
currentfolder = pwd;
if contains(currentfolder,'opto_fp_recordings') %|| contains(event_datapath, 'DH1')
    led_ttl = evt_ts(eventData.Data == 8);
else
...
for i_evt = 1:length(event_check)
    ...
    if length(evt_oe_ts) == length(evt_mpc_ts)
        ...
        if max_dev < max_dev_set
            %fprintf('oe evt %d ts match with mpc %s evt at max deviation %0.2fms, %s \n',evt_c{1},evt_c{2},max_dev*1000, char(events.(sprintf('evt%d',evt_c{1})).type))
        else
        ...
        elseif numel(evt_oe_ts) > numel(evt_mpc_ts)
            ...
            max_dev = abs(diff(evt_oe_ts(1:min_num))-diff(evt_mpc_ts(1:min_num)));
            off_idx = find(max_dev>max_dev_set,1);
            if isempty(off_idx)  % oephys just has extra cue on at end...
                evt_oe_ts(end) = [];
            else
                evt_oe_ts(off_idx+1) = [];
            end
